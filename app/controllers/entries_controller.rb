class EntriesController < ApplicationController
  def index
    ...
    @entries_data    = Entry.select("DISTINCT ON (cast(entry_date as text))").where("cast(entry_date as text) LIKE '#{date_string}%'").group(:category).order('entry_date ASC').sum(:amount)
    ...
    @entries_balance = Entry.select("DISTINCT ON (cast(entry_date as text))").where("cast(entry_date as text) LIKE '#{date_string}%'").group(:balance_type).sum(:amount)
  end
